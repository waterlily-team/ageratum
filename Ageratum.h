 ...
 * @since v0.0.0.12
 */
#define AGERATUM_TWEAK_VERSION 23
...
#include <unistd.h>
    ...
    bool ageratum_writeFile(ageratum_file_t *file, const char *const contents)
    {
        ...
        {
            waterlily_log(ERROR, "Failed to write to file '%s'.",
                          file->filename);
            return false;
        }
                              ...
                              int *status)
    {
        ...
        if (__builtin_expect(pid == 0 && execve(path, (char *const *)trueArgv,
                                                nullptr) == -1,
                             0))
        {
        ...
        {
            waterlily_log(
                WARNING, "File '%s' ended execution with an unexpected result.",
                path);
            return false;
        }
    ...
    bool ageratum_glslToSPIRV(const ageratum_file_t *const file)
    {
        ...
        const char *const argv[] = {
            "--target-env",   "vulkan1.3", "-e",
            "main",           "-g0",       "-t",
            "--glsl-version", "460",       "--spirv-val",
            "--lto",          "--quiet",   "-o",
            outputPath,       path,
        };
